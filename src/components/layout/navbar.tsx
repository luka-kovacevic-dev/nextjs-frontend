            ...
            {ITEMS.map((link) =>
              link.dropdownItems ? (
                <NavigationMenuItem key={link.label} className="">
                  <NavigationMenuTrigger className="data-[state=open]:bg-accent/50 bg-transparent! px-1.5 font-bold">
                    {link.label}
                  </NavigationMenuTrigger>
                  <NavigationMenuContent>
                      ...
                      {link.dropdownItems.map((item) => (
                        <li key={item.title}>
                          <NavigationMenuLink asChild>
                            <Link
                              href={item.href}
                              className="group hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground flex items-center gap-4 rounded-md p-3 leading-normal no-underline outline-hidden transition-colors select-none"
                            >
                ...
                <NavigationMenuItem key={link.label} className="">
                  <Link
                    href={link.href}
                    className={cn(
                      'relative bg-transparent px-1.5 text-sm font-bold',
                      pathname === link.href &&
                        'text-muted-foreground underline underline-offset-7',
                    )}
                  >
                    {link.label}
                  </Link>
